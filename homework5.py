...
# удаляем лишнею запятую в конце строки при ее наличии
    ...
    return new_str
    # Решение окей, можно короче - см ответы 
...
# при выполнении с помощью ф-ции sub библиотеки re заменяем 80 или 375 на +375 и добавляем номер в список
            ...
            lst.append(num)
    return lst
    # Отличное решение - круто, что применяются регулярные выражения.
...
# то применяем ф-цию map для перебора всех значений args и приобразуем к списку
    ...
    return wrapper
    # Все верно, очень хорошо, что применяется функция map, она тут уместна
...
# Очень интересное решение - забрал в эталоны 
